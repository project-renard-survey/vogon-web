...
def update_appellation(field_data, field, cache={}, project_id=None, creator=None, text=None):
	"""
	Some fields may have data for appellations that have not yet been
	created. So we do that here.
	"""
...
def update_relationset(template, raw_data, creator, text, created, project_id=None):
	"""
	Create a new :class:`annotations.models.RelationSet` instance from a
	:class:`annotations.models.RelationTemplate` and user data.
	"""
	...
	def update_relation(relation, name, newId):
		relation.name = newId
		setattr(relation, name, newId)
		relation.save()
		return relation
	# get order of relations and establish what relations are related to each other
	def find_relation_order(relations, order):
		relationName = ['object_object_id', 'source_object_id', 'predicate_id']
		l = len(relations)
		for c, relation in enumerate(relations):
			if c < (l - 1):
				for name in relationName:
					test = getattr(relations[c + 1], name)
					if relation.id == test:
						order.append(relation.id)
		return order
	#pp.pprint(raw_data)
	relationset = RelationSet.objects.get(id=raw_data['relationsetId'])
	relations = Relation.objects.filter(part_of_id=relationset.id)
	if relations.count() == 1:
		relation = relations.first()
		for field in raw_data['fields']:
			if field['part_field'] == 'object':
				if not relation.object_object_id == field['appellation']['id']:
					print('object')
					update_relation(relation, 'object_object_id', field['appellation']['id'])
			if field['part_field'] == 'source':
				if not relation.source_object_id == field['appellation']['id']:
					print('source')
					update_relation(relation, 'source_object_id', field['appellation']['id'])
			if field['part_field'] == 'predicate':
				if not relation.predicate_id == field['appellation']['id']:
					print('predicate')
					update_relation(relation, 'predicate_id', field['appellation']['id'])
	else:
		order = []
		find_relation_order(relations, order)
		print(order)
		temp_parts = RelationTemplatePart.objects.filter(part_of_id=template.id)
		temp_ids = []
		[temp_ids.append(part.id) for part in temp_parts]
		print("Order: {}, Temp: {}".format(order, temp_ids))
		for c, temp in enumerate(temp_ids):
			for field in raw_data['fields']:
				if field['part_id'] == temp:
					reversed(relations)
					relation = relations[c]
					# print(relation.id)
					if field['part_field'] == 'object':
						if 'appellation' in field:
							if relation.object_object_id != field['appellation']['id']:
								print("Temp: {}, Field: {}, Relation: {}, C: {}, Field Id: {}, Id: {}, Object".format(temp, field['part_id'], relation.id, c, field['appellation']['id'], relation.object_object_id))
						else:
							print("No appellation object")
					# 				print("Objects not same {}, {}").format(relation.id, field['appellation']['id'])
					# 	else:
					# 		print("I am not sure")
					# # 		update_relation(relation, 'object_object_id', field['appellation']['id'])
					if field['part_field'] == 'source':
						if 'appellation' in field:
							if relation.source_object_id != field['appellation']['id']:
								print("Temp: {}, Field: {}, Relation: {}, C: {}, Field Id: {}, Id: {}, Source".format(temp, field['part_id'], relation.id, c, field['appellation']['id'], relation.source_object_id))
						else:
							print("No appellation source")
					# 					print("Sources not same {}, {}").format(relation.id, field['appellation']['id'])
					# 		else:
					# 			print("not sure")
					# # 		update_relation(relation, 'source_object_id', field['appellation']['id'])
					if field['part_field'] == 'predicate':
						if 'appellation' in field:
							if relation.predicate_id != field['appellation']['id']:
								print("Temp: {}, Field: {}, Relation: {}, C: {}, Field Id: {}, Id: {}, Predicate".format(temp, field['part_id'], relation.id, c, field['appellation']['id'], relation.predicate_id))
						else:
							print("No predicate appellation")
			...
			# for field in raw_data['fields']:
			# 	if field['part_field'] == 'object':
			# 		if 'appellation' in field and not relation.object_object_id == field['appellation']['id']:
			# 			print("Objects not same")
			# 		else:
			# 			print("I am not sure")
			# 	# 		update_relation(relation, 'object_object_id', field['appellation']['id'])
			# 	if field['part_field'] == 'source':
			# 			if 'appellation' in field and not relation.source_object_id == field['appellation']['id']:
			# 				print("Sources not the same")
			# 			else:
			# 				print("not sure")
			# 	# 		update_relation(relation, 'source_object_id', field['appellation']['id'])
			# 	if field['part_field'] == 'predicate':
			# 		if 'appellation' in field and not relation.predicate_id == field['appellation']['id']:
			# 			print('Predicates not the same')
			# 		else:
			# 			print("Who knows")
